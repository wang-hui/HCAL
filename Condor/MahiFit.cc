...
void MahiFit::phase1Apply(const HBHEChannelInfo& channelData,
  ...
  for(unsigned int iTS=0; iTS<nnlsWork_.tsSize; ++iTS){
    ...
    std::cout << charge << ", " << ped << ", " << tsAdc << ", " << channelData.tsRiseTime(iTS) << ", " << channelData.fcByPE() << ", ";
  ...
  std::cout << reconstructedVals[0] << ", " << channelData.tsGain(0) << ", " << reconstructedEnergy << ", " << channelData.id().rawId() << ", " << channelData.id().subdet() << ", " << channelData.id().depth() << ", " << channelData.id().ieta() << ", " << channelData.id().iphi() << std::endl;
}
