int plot_time()
{
    ...
    if(plot_ieta_vs_depth)
    {
        ...
        auto mean_time_h=new TH2F("mean_time_h","mean_time_h",ieta_last+1-ieta_first,ieta_first,ieta_last+1, depth_last+1-depth_first, depth_first, depth_last+1);
        ...
        for (int i = ieta_first; i <= ieta_last; i++)
        {
            for (int j = depth_first; j <= depth_last; j++)
            {
                ...
                h2->GetXaxis()->SetRangeUser(10,50);
